...
function SWEP:Think()
	...
	if(State==3)then
		...
		if(SERVER)then
			...
			local WeldNorm=nil
			for i=1,2 do
				local TressDat={start=BaseShootPos,endpos=ShootPos+AimVec*math.Rand(100,110)+VectorRand()*math.Rand(0,20),filter=self.EZowner,mask=MASK_SHOT}
		...
		self.EZowner:SetVelocity(-AimVec*.5)
	else
...
if(CLIENT)then
	...
	function SWEP:ViewModelDrawn()
		if(self.dt.State==3)then
			...
			local Dir=self.EZowner:GetAimVector()
	...
	if(IsValid(Wep))then
		if(Wep:GetClass()=="wep_jack_fungun_gamma")then
			if(Wep.dt.State==3)then
				...
				Wep.EZowner:SetAnimation(PLAYER_ATTACK1)
