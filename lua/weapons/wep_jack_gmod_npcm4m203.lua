...
if(SERVER)then
	...
	function SWEP:CanSee(Ent)
		local TressDet={}
		TressDet.start=self:GetOwner():GetShootPos()
		TressDet.endpos=Ent:LocalToWorld(Ent:OBBCenter())
		TressDet.filter={self:GetOwner(),Ent}
	...
	function SWEP:BadGuy()
		if(IsValid(self:GetOwner()))then
			local Dude=self:GetOwner():GetEnemy()
			...
			if not(self.Reloading)then
				...
				if not(IsValid(Enemy))then
					if(IsValid(LastEnemy))then
						self:GetOwner():UpdateEnemyMemory(LastEnemy,LastEnemy:GetPos())
					else
						if not(self:GetOwner().BeingACombatLifeSaver)then
							if(math.random(1,20)==11)then
								for key,dude in ipairs(ents.FindInSphere(self:GetOwner():GetPos(),1000))do
									if(dude.JackyUSSoldier)then
										if not(dude==self:GetOwner())then
											if(dude:Health()<49)then
												self:GetOwner().BeingACombatLifeSaver=true
												self:GetOwner().CLSPatient=dude
											end
										end
									end
								end
							...
							if(IsValid(self:GetOwner().CLSPatient))then
								local Dist=(self:GetOwner():GetPos()-self:GetOwner().CLSPatient:GetPos()):Length()
								if(Dist>50)then
									if not(self:GetOwner():IsCurrentSchedule(SCHED_FORCED_GO_RUN))then
										self:GetOwner():SetLastPosition(self:GetOwner().CLSPatient:GetPos())
										self:GetOwner():SetSchedule(SCHED_FORCED_GO_RUN)
									end
								else
									JackyPlayNPCAnim(self:GetOwner(),"Heal",true,1)
									self:GetOwner().CLSPatient:StopMoving()
									local Wut=self:GetOwner().CLSPatient
									...
									self:GetOwner().BeingACombatLifeSaver=false
									self:GetOwner().CLSPatient=nil
								end
							else
								self:GetOwner().BeingACombatLifeSaver=false
							end
					...
					if(self:GetOwner():IsCurrentSchedule(SCHED_COMBAT_STAND))then
	...
	function SWEP:NPCShoot_Primary(ShootPos,ShootDir)
		if((self.NextShootTime<CurTime())and not(self.Reloading))then
			local Anim=self:GetOwner():GetSequenceName(self:GetOwner():GetSequence())
			...
			if((IsValid(Enem))and((Anim=="shoot_ar2")or(Anim=="ShootToIdleAngry")))then
				local Dist=(Enem:GetPos()-self:GetOwner():GetPos()):Length()
				local Chance=1.25-(Dist/2000)
				if(math.Rand(0,1)<Chance)then
					timer.Simple(.075,function()
						self:GetOwner():SetSchedule(SCHED_RANGE_ATTACK1)
						self:GetOwner():RestartGesture(279)
	...
	function SWEP:Shoot(ShootPos,ShootDir)
		...
		if(self.RoundsInMag>0)then
			...
			Fect:SetStart(ShootPos+ShootDir*40-self:GetOwner():GetUp()*10)
			...
			local Dist=(Enemy:GetPos()-self:GetOwner():GetPos()):Length()
			local Miss=50
			if not(self:GetOwner().USMilitaryRiflemanTraining)then Miss=5 end
			...
			EyeTraceData.filter=self:GetOwner()
			...
			if(EyeTrace.Hit)then
				...
				Bewlat.Tracer=0
				self:GetOwner():FireBullets(Bewlat)
			end
...
function SWEP:Reload()
	...
	JackyPlayNPCAnim(self:GetOwner(),"reload_smg1",true,3)
	self:GetOwner():EmitSound("snd_jack_npcriflereload.wav")
