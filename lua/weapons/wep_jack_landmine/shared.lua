...
SWEP.VElements={
	["mine"]={ type="Model", model="models/props_pipes/pipe02_connector01.mdl", bone="ValveBiped.Bip01_R_Hand", rel="", pos=Vector(7, -3, -1), angle=Angle(-0, -38, -0), size=Vector(.85, .85, .85), color=Color(255, 255, 255, 255), surpresslightning=false, material="models/jacky_camouflage/Digi", skin=0, bodygroup={} }
}
SWEP.WElements={
	["mine"]={ type="Model", model="models/props_pipes/pipe02_connector01.mdl", bone="ValveBiped.Bip01_R_Hand", rel="", pos=Vector(3.099, 6.5, -2.274), angle=Angle(-68.524, 7.158, 0), size=Vector(.85, .85, .85), color=Color(255, 255, 255, 255), surpresslightning=false, material="models/jacky_camouflage/Digi", skin=0, bodygroup={} }
}
...
local CamoTable={"Digi","Grass","Dirt","Sand","Snow","Wood","Concrete","Asphalt","Brick","Black"}
	...
	if(self.dt.State==2)then
		...
		timer.Simple(.5,function()
			if(IsValid(self))then
				...
				if(SERVER)then self:GetOwner():StripWeapon("wep_jack_landmine") end
			end
		end)
...
function SWEP:Think()
	...
	if(SERVER)then
		if(self.dt.State==7)then
			if(self.NextSprayTime<CurTime())then
				self.dt.State=8
				if(IsValid(self.PaintBox))then
					self.PaintBox:ProvideForPaintJob()
					self.PaintBox=nil
				end
					...
					if(IsValid(self))then
						...
						timer.Simple(.5,function()
							if(IsValid(self))then
								...
								umsg.End()
								timer.Simple(.5,function()
									if(IsValid(self))then self.dt.State=2 end
								end)
							end
						end)
					end
	...
	if(Yah.Hit)then
		if(((Yah.HitPos-self:GetOwner():GetShootPos()):Length()<70)and not(Yah.Entity:GetClass()=="ent_jack_minebox"))then
...
function SWEP:Holster()
	if(SERVER)then
		if((self.dt.State==2)or(self.dt.State==4))then
			...
			DroppedMine:SetPos(self:GetOwner():GetShootPos()+self:GetOwner():GetAimVector()*20-self:GetOwner():GetUp()*20)
			DroppedMine.Owner=self:GetOwner()
			DroppedMine.Camo=CamoTable[self.dt.Camo]
			DroppedMine.ArmMode="None"
			DroppedMine:Spawn()
			DroppedMine:Activate()
			DroppedMine:GetPhysicsObject():SetVelocity(self:GetOwner():GetVelocity()*0.5+self:GetOwner():GetAimVector()*10)
			if(SERVER)then self:GetOwner():StripWeapon("wep_jack_landmine") end
		end
	end
	return true
...
function SWEP:Reload()
	...
	local Trayuss=self:GetOwner():GetEyeTrace()
	if(Trayuss.Hit)then
		if(Trayuss.Entity:GetClass()=="ent_jack_minebox")then
			if((Trayuss.HitPos-self:GetOwner():GetShootPos()):Length()<60)then
				...
				if(self.dt.State==7)then
					...
					self:GetOwner():PrintMessage(HUD_PRINTCENTER,CamoTable[self.dt.Camo])
					self.NextSprayTime=CurTime()+1
					self.NextReloadTime=CurTime()+.3
				end
			else
				self:GetOwner():PrintMessage(HUD_PRINTCENTER,"Need Mine Crate")
			end
		else
			self:GetOwner():PrintMessage(HUD_PRINTCENTER,"Need Mine Crate")
		end
...
if(CLIENT)then
	local function Change(data)
		local Wap=data:ReadEntity()
		if(Wap.VElements)then Wap.VElements.mine.material="models/jacky_camouflage/"..CamoTable[Wap.dt.Camo] end
		if(Wap.WElements)then Wap.WElements.mine.material="models/jacky_camouflage/"..CamoTable[Wap.dt.Camo] end
	end
	usermessage.Hook("JackysLandmineCamoChangeUMSG",Change)
end
...
function SWEP:ThrowMine(willArm)
	if(SERVER)then
		...
		Det:SetPos(self:GetOwner():GetShootPos()+self:GetOwner():GetAimVector()*30-self:GetOwner():GetUp()*20)
		...
		local TheAngle=self:GetOwner():GetAimVector():Angle()
		...
		Det:GetPhysicsObject():SetVelocity(self:GetOwner():GetVelocity()*.75+self:GetOwner():GetAimVector()*Speed)
	end
end
...
function SWEP:StickMine(willArm)
	if(SERVER)then
		...
		if(Tr.Hit)then
			if((((self:GetOwner():GetShootPos()-Tr.HitPos)):Length()<100)and not(Tr.Entity:GetClass()=="ent_jack_minebox"))then
				local Det=ents.Create("ent_jack_landmine")
				...
				if(willArm)then
					...
					timer.Simple(1,function() if(IsValid(Det))then Det:SetCollisionGroup(COLLISION_GROUP_NONE) end end)
				end
			else
				self:ThrowMine(willArm)
			end
		else
			self:ThrowMine(willArm)
		end
