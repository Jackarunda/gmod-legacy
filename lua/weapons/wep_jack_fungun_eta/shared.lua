...
function SWEP:ReloadPile()
	...
	timer.Simple(.75,function()
		if(IsValid(self))then
			self.dt.State=3
			self:EmitSound("snd_jack_pilereload.wav",70,100)
			self:SendWeaponAnim(ACT_VM_PRIMARYATTACK)
			self:GetOwner():GetViewModel():SetPlaybackRate(.15)
			if(SERVER)then
				local Poof=EffectData()
				Poof:SetOrigin(self:GetOwner():GetShootPos()+self:GetOwner():GetAimVector()*20+self:GetOwner():GetRight()*3-self:GetOwner():GetUp()*3)
				Poof:SetStart(self:GetOwner():GetVelocity()+self:GetOwner():GetRight()*100)
	...
	timer.Simple(2.8,function()
		if(IsValid(self))then
			self.dt.State=2
			if(self:GetOwner():KeyDown(IN_ATTACK))then self:PrimaryAttack() end
		end
	end)
end
...
function SWEP:PrimaryAttack()
	...
	local ShootPos=BaseShootPos+self:GetOwner():GetRight()-self:GetOwner():GetUp()
	
	local PreTraceData={start=BaseShootPos,endpos=ShootPos,filter=self:GetOwner()}
	...
	MainTraceData.filter=self:GetOwner()
	MainTraceData.mask=MASK_SHOT
	local MainTrace=util.TraceLine(MainTraceData)
	
	sound.Play("snd_jack_pilefire.wav",self:GetOwner():GetShootPos(),75,100)
	self.dt.Extension=1
	self:SendWeaponAnim(ACT_VM_PRIMARYATTACK)
	...
	self:GetOwner():ViewPunch(Angle(math.Rand(-.5,.5),math.Rand(-.5,.5),math.Rand(-.5,.5)))
	...
	if(MainTrace.Hit)then
		...
		if(SERVER)then		
			...
			Dammej:SetAttacker(self:GetOwner())
			Dammej:SetInflictor(self)
			Dammej:SetDamageForce(AimVec*Force)
			MainTrace.Entity:TakeDamageInfo(Dammej)
		end
		
		if(Hardness>.7)then
			self:EmitSound("snd_jack_pileresonate_loud.wav",70,100)
			self.HeldBackAmount=1.5
			self:GetOwner():SetVelocity(-AimVec*225*Proximity)
		else
			self.HeldBackAmount=1.2
			if not((MainTrace.Entity:IsPlayer())or(MainTrace.Entity:IsNPC()))then
				self:GetOwner():SetVelocity(-AimVec*150*Proximity)
			end
		end
	else
		self:EmitSound("snd_jack_pileresonate_quiet.wav",70,100)
...
function SWEP:MakeImpactEffect(pos,dir,num)
	...
	self:GetOwner():FireBullets(EffectBullet)
end
...
function SWEP:Think()
	...
	if(Extension>0)then
		...
		if not(LockPlayed)then
			LockPlayed=true
			if(SERVER)then self:EmitSound("snd_jack_pilelock.wav") end
		end
	end
...
if(CLIENT)then
	local function DrawExtension()
		for key,wep in ipairs(ents.FindByClass("wep_jack_fungun_eta"))do
