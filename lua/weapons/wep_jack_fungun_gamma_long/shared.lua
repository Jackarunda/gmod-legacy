...
function SWEP:Initialize()
	...
	self.BlastingSound=CreateSound(self,"snd_jack_heavyplasmaloop.wav")
...
function SWEP:Think()
	...
	if(State==3)then
		...
		if(SERVER)then
			for i=1,3 do
				local TressDat={start=BaseShootPos,endpos=ShootPos+AimVec*math.Rand(200,220)+VectorRand()*math.Rand(0,30),filter=self:GetOwner(),mask=MASK_SHOT}
		...
		self:GetOwner():SetVelocity(-AimVec*.8)
	else
...
function SWEP:BurstCool()
	...
	if not(BurstCoolSoundPlayed)then
		...
		self:EmitSound("snd_jack_heavylaservent.wav",70,125)
	end
...
local function GunThink()	
	for key,wep in ipairs(ents.FindByClass("wep_jack_fungun_gamma_long"))do
