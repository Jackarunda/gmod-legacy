...
function ENT:SpawnFunction(ply,tr)
	...
	local ent=ents.Create("ent_jack_fgweaponbox_eta")
	ent:SetPos(SpawnPos)
	ent:Spawn()
	ent:Activate()
...
function ENT:Use(activator)
	...
	if not(activator:HasWeapon("wep_jack_fungun_eta"))then
		sound.Play("snd_jack_boxopen.wav",self:GetPos(),70,95)
		activator:EmitSound("BaseCombatCharacter.AmmoPickup")
	
		activator:Give("wep_jack_fungun_eta")
		activator:SelectWeapon("wep_jack_fungun_eta")
