...
function ENT:SpawnFunction(ply,tr)
	...
	local ent=ents.Create("ent_jack_fgweaponbox_gamma_long")
	ent:SetPos(SpawnPos)
	ent:Spawn()
	ent:Activate()
...
function ENT:Initialize()
	self:SetModel("models/mass_effect_3/weapons/misc/armor jase smaller.mdl")
...
function ENT:Use(activator)
	...
	if not(activator:HasWeapon("wep_jack_fungun_gamma_long"))then
		sound.Play("snd_jack_boxopen.wav",self:GetPos(),70,95)
		activator:EmitSound("BaseCombatCharacter.AmmoPickup")
	
		activator:Give("wep_jack_fungun_gamma_long")
		activator:SelectWeapon("wep_jack_fungun_gamma_long")
